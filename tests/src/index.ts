...
function testSingleNativeHash(input: Buffer, expectedLen: number, options: { keyed: Buffer | null; keyed_len: number; hash_length: number }): boolean {
  try {
...
function testStreamingNativeHash(input: Buffer, expectedLen: number, options: { keyed: Buffer | null; keyed_len: number; hash_length: number }): boolean {
  try {
    const inputBuf = input;
    const optionsBuf = HashOptions.encode(options);
    const outBuf = Buffer.alloc(Math.max(expectedLen, 128), 0);
...
function testStreamingXofNativeHash(input: Buffer, expectedLen: number, options: { keyed: Buffer | null; keyed_len: number; hash_length: number }): boolean {
  try {
    const inputBuf = input;
    const optionsBuf = HashOptions.encode(options);
    const outBuf = Buffer.alloc(Math.max(expectedLen, 128), 0);
...
function testStreamingXofWasmHash(input: Buffer, expectedLen: number, options: { keyed?: Uint8Array; derive_key?: string; hash_length?: number }): boolean {
  try {
    const hasher = new blake3.StreamingHasher(options);
    hasher.update(input);
...
function testStreamingBunSha256(input: Buffer, expectedLen: number): boolean {
  try {
    const hasher = new CryptoHasher('sha256');
    hasher.update(input);
    const outBuf = hasher.digest();
...
function testCryptoJsSha256(input: Buffer, expectedLen: number): boolean {
  try {
    const inputStr = input.toString('binary');
    const hash = SHA256(inputStr).toString();
...
async function benchmark() {
  const input = stringToBuffer('Hello, Bun.js!');
  const input1MB = Buffer.alloc(20_000_000, 0x42); // 1MB input
  const iterations = 100;
  const keyed = Buffer.from(new Uint8Array(32).fill(0x42));
  const derive_key = 'context';

  Bun.gc(true); // Gọi GC một lần trước benchmark

  testSingleNativeHash(input, 32, { keyed: null, keyed_len: 0, hash_length: 32 });
  testStreamingNativeHash(input, 32, { keyed: null, keyed_len: 0, hash_length: 32 });
  testStreamingXofNativeHash(input, 64, { keyed: null, keyed_len: 0, hash_length: 64 });

  testSingleWasmHash(input, 32, {});
  testStreamingWasmHash(input, 32, {});
  testStreamingXofWasmHash(input, 64, { hash_length: 64 });

  testSingleBunSha256(input, 32);
  testStreamingBunSha256(input, 32);

  testCryptoJsSha256(input, 32);
  testCryptoJsSha256(input1MB, 32);
  ...
  for (let i = 0; i < iterations; i++) {
    testSingleBunSha256(input1MB, 32);
  }
  ...
  for (let i = 0; i < iterations; i++) {
    testSingleNativeHash(input1MB, 32, { keyed: null, keyed_len: 0, hash_length: 32 });
  }
  console.timeEnd('Native Single Hash (1MB, BLAKE3)');

  console.time('Native Single Hash (Keyed, BLAKE3)');
  for (let i = 0; i < iterations; i++) {
    testSingleNativeHash(input, 32, { keyed, keyed_len: 32, hash_length: 32 });
  }
